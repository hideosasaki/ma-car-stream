...
- Music Assistantからの音声ストリーミング（48kHz/16bit/2ch PCM → MP3 192kbps/44.1kHz）
...
- `INPUT_SAMPLE_RATE`: 48000（snapclient出力）
- `OUTPUT_SAMPLE_RATE`: 44100（MP3出力）
- `CHANNELS`: 2
- `MP3_BITRATE`: 192k
- `HTTP_PORT`: 8000
...
- ffmpegのMP3エンコード+リサンプリングがRaspi 4のCPUを使用する（192kbps/44.1kHzで軽減済み）
