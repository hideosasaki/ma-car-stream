...
Music Assistant (Docker, Raspi4)
  → Snapserver (MA内蔵)
    → snapclient (ホスト側, file出力)
      → FIFO (/tmp/snapstream)
        → icy_server.py (ffmpegでAACエンコード + ICYメタデータ配信)
          → Tailscale
...
- Music Assistantからの音声ストリーミング（48kHz/16bit/2ch PCM → AAC 128kbps/48kHz）
...
- [x] systemdサービス化（Raspi起動時の自動起動）
- [x] MP3 → AACエンコーダ切り替え（CPU負荷・帯域軽減）
...
- snapclientが再接続するとdefaultストリームに戻されることがある（Snapwebで再割り当て）
- FIFOの読み手（icy_server.py内のffmpeg）が先に起動していないとsnapclientがbroken pipeで落ちる（systemdの依存関係で制御済み）
